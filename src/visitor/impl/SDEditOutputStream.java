...
public class SDEditOutputStream {
	...
	public SDEditOutputStream(int depth, String name,
			Hashtable<IMethod, IMethodRelation> methodRelations) {
	...
	public void parse(IMethodRelation start, int depth) {
		...
		classes.append(name + ":"
				+ Utility.simplifyType(start.getFrom().getClassName()) + "\n");
		count++;
		List<IMethod> start_to = start.getTo();
		for (int i = 0; i < start_to.size(); i++) {
			this.content.append(this.declaration.get(start.getFrom()
					.getClassName()));
			this.content.append(":");
			...
			this.content.append(this.declaration.get(start_to.get(i)
					.getClassName()));
			this.content.append(".");
			if (start_to.get(i).getName().equals("init")) {
				...
				// this.content.append(start_to.get(i).getType()+"=");
				// this.content.append(Utility.simplifyClassName(start_to.get(i).getName()));
				// this.content.append(".");
				this.content.append(Utility.simplifyClassName(start_to.get(i)
						.getName()) + "(");
				...
				if (!params.isEmpty())
					this.content.append(Utility.simplifyType(params.get(params
							.size() - 1)) + ")\n");
				else {
					this.content.append(")\n");
				}
	...
	public IMethodRelation getMethodRelationbyName(String name) {
		...
		String params = methodNameAndParams.substring(firstparIndex + 1,
				methodNameAndParams.length() - 1);
		...
		for (IMethodRelation m : this.methodRelations.values()) {
			String newClassName = m.getFrom().getClassName()
					.replaceAll("/", ".");
			if (m.getFrom().getName().equals(method)
					&& newClassName.equals(className)) {
				...
				for (int i = 0; i < m.getFrom().getParamTypes().size(); i++) {
					String param = Utility.simplifyClassName(m.getFrom()
							.getParamTypes().get(i));
	...
	public String toString() {
		return this.classes.toString() + this.content.toString();
